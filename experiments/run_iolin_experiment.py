# experiments/run_iolin_experiment.py
# This script will run the full IOLIN experiment.
# It will load the processed data, instantiate the IOLIN algorithm,
...
from iolin import IOLIN

def run_experiment():
    """Runs the full IOLIN experiment and saves the results."""
    print("--- Starting Full IOLIN Experiment ---")
    ...
    # Initialize IOLIN processor
    iolin_processor = IOLIN(input_cols_with_uniques, target_col, significance_level=SIGNIFICANCE_LEVEL)
    ...
    for i, result in enumerate(iolin_processor.process_data_stream(df, WINDOW_SIZE, STEP_SIZE)):
        ...
        print(f"  Processing window {i+1}/{total_windows}: Status={status}, Time={result['processing_time_s']:.2f}s")
        results.append(result)

    end_time_total = time.time()
    print(f"\n--- IOLIN Experiment Finished ---")

    if results:
        results_df = pd.DataFrame(results)
        # Save results to CSV
        results_path = os.path.join(os.path.dirname(__file__), '..', 'results', "iolin_metrics.csv")
